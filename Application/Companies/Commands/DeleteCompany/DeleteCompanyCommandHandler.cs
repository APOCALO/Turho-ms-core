...
namespace Application.Companies.Commands.DeleteCompany
{
    internal sealed class DeleteCompanyCommandHandler : ApiBaseHandler<DeleteCompanyCommand, Unit>
    {
        ...
        {
            if (await _companyRepository.GetByIdAsync(new CompanyId(request.Id), cancellationToken) is not Company company)
            {
                return Error.NotFound("DeleteCompanyCommandHandler.NotFound", $"Company with ID {request.Id} was not found.");
            }

            if (company.CompanyPhotos.Any())
            {
                await DeletePhotosAsync(company.Id.Value, company.CompanyPhotos);
            }
